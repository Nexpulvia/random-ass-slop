...
------------------------------------------------------------------------
-- SETTINGS: Change your keybinds here (Must be lowercase strings)
------------------------------------------------------------------------
local Config = {
	FlyKey     = "j", -- Key to toggle flight
	DescendKey = "q", -- Key to descend while flying
	MoveUp     = "w", -- Forward
	MoveLeft   = "a", -- Left (Double-tap to barrel roll)
	MoveDown   = "s", -- Backward
	MoveRight  = "d", -- Right (Double-tap to barrel roll)
}
------------------------------------------------------------------------
...
local function runWingScript(char)
	-- Ensure the core torso exists before building
	local tor = char:WaitForChild("Torso", 5)
	...
	function fly()
		...
		function applySkin(skinKey)
			...
			if skinKey == "0" then
				tcol = invcol(char.Torso.BrickColor)
				fcol = char.Torso.BrickColor
				trans, ref, hasFire, fmc, fsc = 0, 0, false, Color3.new(), Color3.new()
			else
				local spec = special[skinKey]
				tcol = spec[1] and BrickColor.new(spec[1]) or invcol(char.Torso.BrickColor)
				fcol = spec[2] and BrickColor.new(spec[2]) or char.Torso.BrickColor
		...
		local feather
		feather = newfeather() feather.Size = Vector3.new(.4,3,.3) weld(r4,feather,CFrame.new(-.1,-.3,0),CFrame.new(0,-1.5,0))
		feather = newfeather() feather.Size = Vector3.new(.4,2.3,.3) weld(r4,feather,CFrame.new(.1,-.1,0) * CFrame.Angles(0,math.random()*.1,0),CFrame.new(0,-1.1,0))
		feather = newfeather() feather.Size = Vector3.new(.35,2.2,.25) weld(r4,feather,CFrame.new(.1,-.3,0) * CFrame.Angles(0,math.random()*.1,math.rad(-10)),CFrame.new(0,-1.1,0))
		...
		feather = newfeather() feather.Size = Vector3.new(.4,3,.3) weld(l4,feather,CFrame.new(-.1,-.3,0),CFrame.new(0,-1.5,0))
		feather = newfeather() feather.Size = Vector3.new(.4,2.3,.3) weld(l4,feather,CFrame.new(.1,-.1,0) * CFrame.Angles(0,math.random()*.1,0),CFrame.new(0,-1.1,0))
		feather = newfeather() feather.Size = Vector3.new(.35,2.2,.25) weld(l4,feather,CFrame.new(.1,-.3,0) * CFrame.Angles(0,math.random()*.1,math.rad(-10)),CFrame.new(0,-1.1,0))
		...
		kd = plr:GetMouse().KeyDown:connect(oc(function(key) 
			...
			-- Map the incoming dynamic keys to standard states based on setup configuration
			local mappedKey = nil
			if key == Config.MoveUp then mappedKey = "w"
			elseif key == Config.MoveLeft then mappedKey = "a"
			elseif key == Config.MoveDown then mappedKey = "s"
			elseif key == Config.MoveRight then mappedKey = "d"
			elseif key == Config.DescendKey then mappedKey = "q"
			end

			if mappedKey then
				keysdown[mappedKey] = true 
				keypressed[mappedKey] = true
			end

			if key == Config.DescendKey then 
				descendtimer = tick() 
			elseif key == Config.FlyKey then 
				toggleFlight()
			elseif (key == Config.MoveLeft or key == Config.MoveRight) and ktime[key] and tick()-ktime[key] < .3 then
				-- Forced spin initialization via dynamic configuration key matches
				reqrotx = key == Config.MoveLeft and math.pi*2 or -math.pi*2
			end
			ktime[key] = tick() 
		end))
		...
		ku = plr:GetMouse().KeyUp:connect(function(key) 
			key = key:lower()
			if key == Config.MoveUp then keysdown["w"] = false
			elseif key == Config.MoveLeft then keysdown["a"] = false
			elseif key == Config.MoveDown then keysdown["s"] = false
			elseif key == Config.MoveRight then keysdown["d"] = false
			elseif key == Config.DescendKey then keysdown["q"] = false
			end
		end)
		...
		mainButton.Position = UDim2.new(0.85, -65, 0.5, -25)
		...
		con = game:GetService("RunService").Stepped:connect(oc(function()
			if not char or not char:FindFirstChild("Torso") or not hum or hum.Health <= 0 then 
				con:Disconnect() 
				return 
			end
			...
			if flying then
				if keysdown.a and math.abs(reqrotx) > 0.5 then
					reqrotx = math.pi * 2
				elseif keysdown.d and math.abs(reqrotx) > 0.5 then
					reqrotx = -math.pi * 2
				else
